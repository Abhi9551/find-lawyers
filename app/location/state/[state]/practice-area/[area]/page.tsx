...
export default function StatePracticeAreaPage({ params }: { params: { state: string; area: string } }) {
  // Format state name for display (e.g., "new-york" -> "New York")
  const stateName = params.state
    .split("-")
    .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
    .join(" ")

  // Format practice area for display (e.g., "criminal-defense" -> "Criminal Defense")
  ...
  // Filter lawyers based on state and practice area
  useEffect(() => {
    // In a real app, this would be an API call with the state and practice area as parameters
    const filteredLawyers = lawyersData.filter(
      (lawyer) =>
        lawyer.location.toLowerCase().includes(stateName.toLowerCase()) &&
        lawyer.practiceAreas.toLowerCase().includes(params.area.replace(/-/g, " ").toLowerCase()),
    )
    setLawyers(filteredLawyers)
    setFilteredLawyers(filteredLawyers)
  }, [stateName, params.area])
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="mb-8" data-aos="fade-up">
            <h1 className="mb-2 text-3xl font-bold">
              {areaName} Lawyers in {stateName}
            </h1>
            <p className="text-lg text-slate-600">
              Find the best {areaName.toLowerCase()} attorneys in {stateName} for your legal needs
            </p>
          </div>
              ...
              {filteredLawyers.length > 0 ? (
                ...
                <div className="rounded-lg bg-white p-8 text-center shadow" data-aos="fade-up">
                  ...
                  <p className="text-muted-foreground">
                    No {areaName.toLowerCase()} lawyers in {stateName} match your current filters. Try adjusting your
                    search criteria.
                  </p>
                </div>
              )}
